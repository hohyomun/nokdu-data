...
def load_data_smartly(uploaded_file, sheet_keywords, header_keywords):
    ...
    if header_idx != -1:
        ...
        # 优化清洗逻辑：只要表头关键字段还有任意一个有值，就不抛弃这行（解决26年目标/实绩抓取丢失问题）
        ...
        for kw in valid_cols:
            mask = mask | df[kw].astype(str).str.contains('小计|合计|总计', na=False)
        df = df[~mask]
...
def style_dataframe(df, float_cols=None):
    ...
    for col in numeric_cols:
        if any(f_c in col for f_c in float_cols): # 如果列名包含"单价"等，则保留两位小数
            format_dict[col] = "{:,.2f}"
        else:
            format_dict[col] = "{:,.0f}"
...
def show_dashboard():
    ...
    with tab1:
        if not df_sales.empty:
            ...
            if not df_history.empty:
                ...
                # 按照年份提取数字并进行降序排列 (2025 -> 2024 -> 2023)
    ...
    with tab2:
        if not df_inventory.empty:
            ...
            fig_val = px.bar(df_top_val, x='在库金额', y='产品名称', orientation='h', 
                             color='在库金额', color_continuous_scale='Blues', text='产品名称')
            fig_val.update_layout(
                plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', 
                yaxis={'categoryorder':'total ascending', 'showticklabels': False, 'title': ''}, 
                margin=dict(t=10, b=0, l=10, r=20), height=380
            )
            fig_val.update_traces(textposition='inside', insidetextanchor='start', textfont=dict(color='white', size=13))
            st.plotly_chart(fig_val, use_container_width=True)
            
            # 【优化】饼图：废弃图例，将文字包围在圆饼四周
            ...
            fig_qty.update_layout(
                paper_bgcolor='rgba(0,0,0,0)', margin=dict(t=20, b=20, l=20, r=20), height=600,
                showlegend=False  # 隐藏底部独立图例
            )
            # 文字信息环绕在扇区外面，显示全称和百分比
            fig_qty.update_traces(textinfo='label+percent', textposition='outside', textfont=dict(size=12))
            st.plotly_chart(fig_qty, use_container_width=True)
            
            st.markdown("<hr>", unsafe_allow_html=True)
            st.markdown('<div class="section-title">📋 详细成品库存台账明细</div>', unsafe_allow_html=True)
            
            # 使用 float_cols=['单价'] 指定单价保留两位小数

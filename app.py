...
def load_data_smartly(uploaded_file, sheet_keywords, header_keywords):
    ...
    if header_idx != -1:
        ...
        # 【全方位强力除杂】不只查关键字列，横向扫描全表前10列，彻底防止“小计”写在品牌列导致遗漏
...
def show_dashboard():
    ...
    # ═══════════════════════════════════════════
    # Tab 2: 成品库存 (核心问题已彻底修复)
    # ═══════════════════════════════════════════
    with tab2:
        if not df_inventory.empty:
            df_inv_clean = df_inventory.copy()
            
            # 去除格式符号强制转数字，保障加和准确无误
            ...
            if not df_top_val.empty:
                ...
                # 舍弃看不清的渐变色，采用高质感商务深蓝，配白字，视觉极其锐利直观
                fig_val = px.bar(df_top_val, x='在库金额', y='产品简称', orientation='h', text='产品名称')

...
def show_dashboard():
    ...
    # ═══════════════════════════════════════════
    # Tab 2: 成品库存 (全面优化强转数值，杜绝图表消失)
    # ═══════════════════════════════════════════
    with tab2:
        if not df_inventory.empty:
            df_inv_clean = df_inventory.copy()
            
            # 【核心修复】：强行摘掉Excel自带的千位逗号，确保金额和数量全部转为真实数字！
            ...
            for dc in date_cols:
                ...
                df_inv_clean[dc] = df_inv_clean[dc].fillna("") # 处理转换失败出现的NaT
            ...
            if not df_top_val_pie.empty:
                fig_qty = px.pie(df_top_val_pie, names='产品名称', values='在库金额', hole=0.5)
            ...
            # 【修复精度】：使用 float_cols=['单价'] 指定单价保留两位小数
